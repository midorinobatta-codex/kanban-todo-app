              ...
              <>
                ...
                {visibleProjects.map((project) => (
                  <article
                    ...
                    className="cursor-pointer rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:border-slate-300 hover:bg-slate-50/60 focus:outline-none focus:ring-2 focus:ring-slate-300"
                  >
...
function InfoChip({
  ...
  return (
    <div className={`rounded-xl px-4 py-3 ${danger ? 'bg-rose-50' : 'bg-slate-50'}`}>
      <p className="text-xs text-slate-500">{label}</p>
      <p className={`mt-1 text-sm font-medium ${danger ? 'text-rose-700' : 'text-slate-900'}`}>
        {value}
      </p>
    </div>
  );
